...
# Colors
SCREEN_BACKGROUND_WHITE = (203,240,208)
COOKIE_BROWN = (159,81,32)
DEVICES_GRAY = (120,120,120)
BELT_BLACK = (0,0,0)
TEXT_BLACK = (0,0,0)
CAMERA_AREA_RED = (200,0,0)
CHOCOLATE_CHIP_BROWN = (60,6,6)

#Lists
BAD_COLORS = [(23, 9, 3), (40,18,5), (70,20,15), (74, 53, 42), (169, 111, 90)]
BAD_SHAPES = [(0,0,0), (1,2,15), (3,5,15), (4,6,20)]
CHIP_COORDINATES = [(35,26), (29,40), (41,38), (18,33), (24,19), (47,18), (52,33), (21,48), (37,52), (51,48)]
POLYGON_ANGLE = [20, 60, 45, 140, 160, 180, 220, 240, 270, 315, 340, 350]
POLYGON_POSITION = [(18,18), (20,30), (45,21), (31,25), (28,39)]
...
CAM_POSITION = COOKIE_WIDTH * 4 + CAM_MARGIN + 100
BELT_SPEED = 200 #between 30 and 300
...
class Cookie:
    def __init__(self, x, y):
        ...
        self.isBad = 0<self.bad<7
        self.chip_amount = 0
        self.polygon = False
        self.polygon_amount = random.randint(2,12)
        self.polygon_angle = 0
        self.shape_arc = (0,0,0)

        if self.bad == 1 or self.bad == 4:
            self.color = BAD_COLORS[random.randint(0,4)]
        else:
            self.color = COOKIE_BROWN
        
        if self.bad == 5 or self.bad == 6:
            self.chip_amount = random.randint(1,3)
        else:
            self.chip_amount = 10

        if self.bad == 2: 
            self.shape_arc = BAD_SHAPES[random.randint(0,3)]
        if self.bad == 3:
            self.polygon = True
            self.chip_amount = random.randint(2,3)
   
    def draw(self, screen):
        
        if(self.polygon == True):
            # Draw cookie pieces
            for x in range(self.polygon_amount):
                self.polygon_angle = POLYGON_ANGLE[x]
                rad = self.polygon_angle * (3.14159 / 180)
                x_offset = self.x + COOKIE_WIDTH * abs(math.cos(rad))
                y_offset = self.y + COOKIE_WIDTH * abs(math.sin(rad))
                pygame.draw.polygon(screen, self.color, ((self.x+POLYGON_POSITION[x//2-2][0], self.y+POLYGON_POSITION[x//2-2][1]), (x_offset, y_offset), (self.x+45, self.y+52), (self.x+25, self.y+42)))

            # Draw chocolate chips on cookie pieces
            for x in range(self.chip_amount):
                pygame.draw.circle(screen, CHOCOLATE_CHIP_BROWN, (self.x+CHIP_COORDINATES[x][0], self.y+CHIP_COORDINATES[x][1]), 5)
            ...
            # Draw chocolate chips on cookies
            for x in range(self.chip_amount):
                pygame.draw.circle(screen, CHOCOLATE_CHIP_BROWN, (self.x+CHIP_COORDINATES[x][0], self.y+CHIP_COORDINATES[x][1]), 5)
